...
public class reportController implements Initializable {
    ...
    private void loadEmployees() {
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            while (rs.next()) {
                employees.add(new Employee(rs.getInt("employee_id"), rs.getString("employee_name"),
                        rs.getString("employee_email"), rs.getString("phone_number"), rs.getString("employee_position"),
                        rs.getString("employee_address"), rs.getDouble("salary"), rs.getString("employee_gender"),
    ...
    private void loadProducts() {
        ...
        try (Connection conn = DatabaseConnection.connect();
             ...
             PreparedStatement stmtAggregates = conn.prepareStatement(queryAggregates);
             ResultSet rsAggregates = stmtAggregates.executeQuery()) {

            productData.clear();

            while (rsDetails.next()) {
                Product product = new Product(rsDetails.getString("product_name"), rsDetails.getInt("product_id"),
                        rsDetails.getInt("quantity"), rsDetails.getDouble("cost"), rsDetails.getInt("storage_id"));
                productData.add(product);
            }

            if (rsAggregates.next()) {
                numItems.setText(String.valueOf(rsAggregates.getInt("TotalItems")));
                totalCostItems.setText(String.format("%.2f", rsAggregates.getDouble("TotalCost")));
            }
    ...
    private void loadSupplierData() {
        ...
        String countQuery = "SELECT COUNT(*) AS TotalSuppliers FROM supplier";
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            while (rs.next()) {
                Supplier supplier = new Supplier(rs.getInt("supplier_id"), rs.getString("supplier_name"),
                        rs.getString("supplier_address"), rs.getInt("phone_number"), rs.getString("supplier_email"));
                supplierData.add(supplier);
            }
            supplierTable.setItems(supplierData);

            if (countRs.next()) {
                numSuppliers.setText(String.valueOf(countRs.getInt("TotalSuppliers")));
            }
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            while (rs.next()) {
                Customer customer = new Customer(rs.getInt("customer_id"), rs.getString("customer_name"),
                        rs.getString("customer_address"), rs.getString("customer_email"), rs.getInt("phone_number"));
    ...
    private void loadEquipmentData() {
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            while (rs.next()) {
                Equipment equipment = new Equipment(rs.getInt("equipment_id"), rs.getString("equipment_name"),
            ...
            showAlert("Database Error", "Failed to load equipment: " + ex.getMessage(), Alert.AlertType.ERROR);
            ex.printStackTrace();
        }
    }
    ...
    private void displayTopCustomer() {
        String query = """
                SELECT c.customer_name, COUNT(s.product_id) AS total_purchases
                FROM customer c
    ...
    private void loadMostPurchasedProduct() {
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            showAlert("Database Error", "Failed to load data: " + ex.getMessage(), Alert.AlertType.ERROR);
            ex.printStackTrace();
        }
    }
}
